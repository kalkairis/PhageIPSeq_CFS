...
from PhageIPSeq_CFS.config import visualizations_dir, oligo_families
...
def create_figure_1(overwrite=True):
    ...
    ax.set_axis_off()
    internal_spec = spec[0, 1].subgridspec(1, 2, wspace=0)
    ...
    ax = fig.add_subplot(internal_spec[1], sharey=ax)
    ...
    internal_spec = spec[0, 2].subgridspec(1, 2, wspace=0)
    ax = fig.add_subplot(internal_spec[0])
